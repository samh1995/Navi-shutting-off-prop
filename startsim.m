...
%     display(iSim)   
    ...
    if Control.accelRefCalculated*SimParams.useRecovery == 1 %recovery control     
       if SimParams.useFaesslerRecovery == 1  
            ...
            [Control] = computedesiredacceleration(Control, Twist); %control.rpmshutdown is here!

            % Compute control outputs
            [Control] = controllerrecovery(tStep, Pose, Twist, Control);    %turn rpm4-0 if rmpshutdown is 1   
...
%     rpmshutdown=Control.rpmshutdown
    %% Propagate Dynamics
...
%    x=R1
    ...
    if SimParams.recordContTime == 0
        ...
        [stateDeriv, Contact, PropState] = dynamicsystem(tODE(end),stateODE(end,:), ...
                                                         tStep,Control.rpm,ImpactParams, PropState.rpm, ...
                                                         Experiment.propCmds,PropState.shutdown);        
